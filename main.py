...
def run_program():
	...
	game_window = tk.Frame(root, width=config.play_screen_width, height=config.play_screen_height, borderwidth=1, relief="solid")
	...
	game_window_button.config(command=lambda: tetris_game.run_game(config, stats, scoreboard_window, user_board_window, int((root.winfo_screenwidth() - config.screen_width)/2), 0))
	
	scoreboard_window = ScoreBoard(root, config, stats)
	scoreboard_window.place_scoreboard()
	
	user_board_window = UserBoard(root, config, stats)
